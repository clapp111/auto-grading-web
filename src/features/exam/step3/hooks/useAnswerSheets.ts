...
export function useAnswerSheets(examId: number) {
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ['answer-sheets', examId] })
  ...
  const patchMutation = useMutation({
    mutationFn: ({ sheetId, body }: { sheetId: number; body: AnswerSheetPatchRequest }) =>
      ...
      if (updated) {
        qc.setQueryData(
          ...
          (old: AnswerSheetResponse[] | undefined) =>
            old?.map((s) => (s.answer_sheet_id === updated.answer_sheet_id ? updated : s)),
        )
      }
    },
  ...
  const saveIdRegionsMutation = useMutation({
    mutationFn: (body: IdRegionSaveRequest) => sheetsApi.saveIdRegions(examId, body),
    onSuccess: (res) => {
      if (res.data?.job_id) {
  ...
  const sheets = data ?? []
  const matchedCount = sheets.filter((s) => !!s.student_name && !!s.student_no).length
