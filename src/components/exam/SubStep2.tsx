...
export function SubStep2({
  ...
  const [selectedLanguage, setSelectedLanguage] =
    useState<ProgrammingLanguage>("CPP");
  ...
  // OCR 대상 문제 (서술형·손코딩)
  const ocrProblems = problems.filter((p) =>
    OCR_REQUIRED_TYPES.includes(p.type),
  );
  ...
  const serverText =
    modelAnswers.find((a) => a.problem_id === activeTab)?.model_answer_text ??
    "";
  useEffect(() => {
  ...
  const activeProblem =
    ocrProblems.find((p) => p.problem_id === activeProblemId) ?? null;
  ...
  const handleDrawComplete = (selection: DrawSelection) => {
    ...
    runOcr({
      ...
      body: {
        ...
        ...(isCoding ? { language: selectedLanguage } : {}),
      },
    });
  };

  const tabProblem =
    ocrProblems.find((p) => p.problem_id === activeTab) ?? null;
  ...
  const tabLang: ProgrammingLanguage | null =
    tabProblem?.language ??
    (tabProblem?.problem_id === activeProblemId && isCoding
      ? selectedLanguage
      : null);
        ...
        <div className="flex-[1.05] border-l border-[#f0f1f4] flex flex-col min-w-0">
          <div className="px-[22px] pt-[18px] flex-none">
            <div className="flex items-center justify-between mb-[13px]">
              ...
              {ocrRunning && (
                <span className="text-[12px] text-[#9aa0ab] animate-pulse">
                  OCR 처리 중...
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-[7px] px-[14px] h-[42px] bg-[#fafbfc] border-b border-[#eef0f3] flex-none">
              ...
              {tabProblem?.type === "CODING" && (
                <div className="ml-auto relative">
                  ...
                  <ChevronDown
                    size={9}
                    className="absolute right-[3px] top-1/2 -translate-y-1/2 text-[#d97706] pointer-events-none"
                  />
                </div>
              )}
            </div>
                ...
                <div className="flex-1 min-h-0 overflow-hidden">
                  <CodeEditor
                    ...
                    onBlur={() => {
                      if (
                        tabProblem &&
                        localText !== (tabAnswer?.model_answer_text ?? "")
                      ) {
                        updateModelAnswer({
