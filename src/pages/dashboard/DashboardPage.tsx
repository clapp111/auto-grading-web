...
import { z } from 'zod'
import {
  Sparkles, Plus, Search, SlidersHorizontal,
  MoreHorizontal, Users, Clock, X, Trash2,
} from 'lucide-react'
...
const createSchema = z.object({
  name: z.string().min(1, '시험 이름을 입력하세요').max(100, '100자 이내로 입력하세요'),
    ...
    >
      ...
      >
        <div className="flex items-start justify-between px-[26px] pt-[24px]">
          <div>
            <p className="text-[20px] font-extrabold text-[#15171d] tracking-[-0.02em]">새 시험 만들기</p>
            <p className="text-[13.5px] text-[#71757e] mt-1">시험을 만든 뒤 문제지와 답안지를 설정합니다</p>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit(d => mutation.mutate(d))}>
          <div className="px-[26px] pt-[22px]">
            ...
            <label className="block text-[13.5px] font-semibold text-[#3a3e46] mb-[7px] mt-[18px]">설명</label>
            <textarea
            ...
            />
            <p className="text-[12px] text-[#aab0ba] text-right mt-1">{descLen} / 200</p>
          </div>
...
// ── Exam Card ──────────────────────────────────────────────────────────────
function ExamCard({ exam, onDelete }: { exam: ExamResponse; onDelete: (id: number) => void }) {
  const navigate = useNavigate()
  ...
  return (
    ...
    >
          ...
          {menuOpen && (
            ...
            >
              <button
                ...
                onClick={() => { setMenuOpen(false); onDelete(exam.exam_id) }}
              >
                <Trash2 size={13} />
                삭제
              </button>
      ...
      <p className="text-[18px] font-bold text-[#15171d] tracking-[-0.02em] mb-[5px] line-clamp-1">{exam.name}</p>
      <p className="text-[13.5px] text-[#71757e] leading-[1.5] mb-[18px] min-h-[40px] line-clamp-2">
        {exam.description ?? '설명 없음'}
      </p>

      <div className="flex items-center justify-between mb-[7px]">
        <span className="text-[12.5px] text-[#9aa0ab] font-medium">{cfg.progLabel}</span>
        <span className="text-[12.5px] font-bold" style={{ color: pctColor }}>{cfg.pct}%</span>
      </div>
      <div className="h-[7px] rounded-[5px] bg-[#eef0f3] overflow-hidden mb-[15px]">
        <div className="h-full rounded-[5px]" style={{ width: `${cfg.pct}%`, backgroundColor: cfg.barColor }} />
      </div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const h = (e: MouseEvent) => {
      if (!filterDropRef.current?.contains(e.target as Node) && !filterBtnRef.current?.contains(e.target as Node))
        setFilterOpen(false)
    }
    document.addEventListener('mousedown', h)
    return () => document.removeEventListener('mousedown', h)
  ...
  const activeFilterLabel = FILTER_OPTIONS.find(o => o.value === statusFilter)?.label ?? '전체'
    ...
    <>
      <div className="h-screen flex flex-col bg-[#f7f8fa] overflow-hidden">
        ...
        <div className="h-[62px] flex-none bg-white border-b border-[#ecedf1] flex items-center px-[26px] gap-[22px]">
          <div className="flex items-center gap-2 flex-none cursor-pointer" onClick={() => navigate('/')}>
          ...
          >
            ...
            <div className="leading-[1.25]">
              ...
              <p className="text-[11.5px] text-[#9aa0ab]">{displayMember?.affiliation ?? ''}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-[34px] py-[30px]">
          ...
          <div className="flex items-end justify-between mb-[24px]">
            <div>
              <h1 className="text-[27px] font-extrabold text-[#15171d] tracking-[-0.03em]">내 시험</h1>
            ...
            <div className="flex items-center gap-[10px]">
              ...
              <div className="relative">
                ...
                >
                  ...
                  상태
                  {statusFilter !== 'ALL' && (
                    <span className="text-[12px] font-bold text-accent">· {activeFilterLabel}</span>
                  )}
                </button>
                {filterOpen && (
                  <div
                    ...
                    {FILTER_OPTIONS.map(o => (
                      <button
                        ...
                        className={cn(
                          ...
                          statusFilter === o.value ? 'font-bold text-accent' : 'text-[#3a3e46]',
                        )}
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 gap-1">
              <p className="text-[16px] font-semibold text-[#9aa0ab]">시험이 없습니다</p>
              <p className="text-[13.5px] text-[#b0b4bc]">새 시험을 만들어 시작하세요</p>
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-[18px]">
              {exams.map(exam => (
