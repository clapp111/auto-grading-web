...
type PendingPhoto =
  { type: "upload"; dataUrl: string } | { type: "reset" } | null;
    ...
    mutationFn: async (data: ProfileForm) => {
      ...
      if (pendingPhoto?.type === "upload") {
        ...
        if (!s3Res.ok) {
          ...
          throw new Error(
            `S3 업로드 실패 (${s3Res.status})${text ? `: ${text}` : ""}`,
          );
        }
        profileUrl = file_key;
    ...
    onSuccess: (res) => {
      if (res.data && token) {
        ...
        reset({
          ...
          affiliationRole: res.data.affiliation_role,
        });
      ...
      <div className="h-screen flex flex-col bg-white overflow-hidden">
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="w-[210px] flex-none border-r border-[#f0f1f4] p-[20px_14px] flex flex-col gap-[3px]">
            <button
              className={navCls("profile")}
              onClick={() => setTab("profile")}
            >
              프로필
            </button>
            <button
              className={navCls("password")}
          ...
          <div className="flex-1 p-[30px_36px] overflow-y-auto min-w-0">
                    ...
                    {popoverOpen && (
                      ...
                      >
                        ...
                        >
                          <Upload
                            size={14}
                            className="text-[#9aa0ab] flex-none"
                          />
                          파일에서 업로드
                        </button>
                        {hasPhoto && (
                ...
                <form onSubmit={handleSubmit((d) => mutation.mutate(d))}>
                  ...
                  <div className="flex gap-[14px]">
                    <div className="flex-[1.4] min-w-0">
                      <label className="block text-[13px] font-semibold text-[#3a3e46] mb-[7px] mt-4">
                        소속
                      </label>
                      <input
                        type="text"
            ...
            {tab === "password" && (
              <div className="max-w-[480px]">
                <p className="text-[18px] font-extrabold text-[#15171d] tracking-[-0.02em] mb-[28px]">
                  비밀번호 변경
                </p>
                <form
                  onSubmit={handlePwSubmit((d) => pwMutation.mutate(d))}
                  noValidate
                >
                  {/* 현재 비밀번호 */}
                  <label className="block text-[13px] font-semibold text-[#3a3e46] mb-[7px]">
                    현재 비밀번호
                  </label>
                  <div className="relative">
                    <input
                      ...
                      className={cn(
                        inputCls(!!pwErrors.currentPassword),
                        "pr-11",
                      )}
                    />
                    <button
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={cn(
                        inputCls(!!pwErrors.confirmPassword),
                        "pr-11",
                      )}
                    />
                    <button
