    ...
    <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      <div className="px-[30px] py-[24px] pb-[20px] border-b border-[#f0f1f4] flex-none">
        ...
        <div className="flex items-center gap-[12px] mt-[16px]">
          <span className="text-[12.5px] text-[#9aa0ab] font-semibold flex-none">
            전체 채점
          </span>
...
function AutoGradeDetailView({
  ...
  const pct =
    totalStudentCount > 0
      ? Math.round((confirmedCount / totalStudentCount) * 100)
      : 0;
  ...
  useEffect(() => {
    ...
    setLocalCorrect(
      currentGrade.status === "CONFIRMED"
        ? currentGrade.score === problem.max_score
        : null,
    );
  ...
  // 해당 문제의 답안 영역
  const problemRegion =
    sheetRegions.find((r) => r.problem_id === problem.problem_id) ?? null;
  const regionOverlay: RegionOverlay[] = problemRegion?.bbox_region
    ? [
        {
          region: problemRegion.bbox_region,
          label: problem.label,
          color: TYPE_COLORS[problem.type],
          shape: problemRegion.shape,
          polygon_points: problemRegion.polygon_points ?? undefined,
        },
      ]
    : [];
  ...
  const handleConfirm = () => {
    ...
    if (!currentGrade) {
      if (selectedSheet?.student_id != null)
        onCreateGrade(selectedSheet.student_id, score);
    } else {
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0">
            ...
            <div className="flex-1 px-[24px] py-[20px] flex flex-col gap-[16px] overflow-y-auto min-h-0">
              ...
              {correctAnswerText && (
                <div className="border border-[#ebedf1] rounded-[11px] px-[16px] py-[13px]">
                  <div className="text-[12px] text-[#9aa0ab] font-semibold mb-[5px]">
                    모범답안
                  </div>
                  <div className="text-[15px] font-bold text-[#15171d]">
                    {correctAnswerText}
                  </div>
                </div>
              )}
            ...
            <div className="flex-none px-[24px] py-[14px] border-t border-[#f0f1f4] flex gap-[10px]">
              ...
              <button
                ...
                disabled={
                  isProcessing ||
                  (!(isCurrentConfirmed && !editMode) && localCorrect === null)
                }
...
function LlmGradeDetailView({
  ...
  const handleToggleRubric = (rubricId: number, satisfied: boolean) => {
    ...
    const updated = currentBreakdown.map((r) =>
      r.rubric_id === rubricId
        ? { rubric_id: r.rubric_id, satisfied }
      ...
      {isGrading ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-[18px]">
          ...
          <div className="text-center">
            <p className="text-[15px] font-bold text-[#15171d] mb-[6px]">
              LLM이 채점하고 있습니다
            </p>
            {gradingJobProgress && gradingJobProgress.total > 0 ? (
              <>
                ...
                <p className="text-[13.5px] text-[#71757e]">
                  {gradingJobProgress.current} / {gradingJobProgress.total} 채점
                  완료
                </p>
              </>
            ) : (
            ...
            {problem.type === "CODING" ? (
              ...
              <div className="flex-1 mx-[24px] mb-[18px] border border-[#e6e8ec] rounded-[11px] bg-[#fcfcfd] p-[15px_16px] text-[14px] text-[#3a3e36] leading-[1.85] overflow-y-auto min-h-0 font-sans whitespace-pre-wrap">
                {selectedGrade.ocr_text ?? (
                  <span className="text-[#c2c6cd] italic">
                    OCR 인식 텍스트가 없습니다
                  </span>
                )}
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-[24px] flex flex-col gap-[9px] min-h-0">
              ...
              {selectedGrade.comment && (
                <div className="mt-[4px]">
                  <div className="text-[12.5px] text-[#9aa0ab] font-semibold mb-[7px]">
                    LLM 코멘트
                  </div>
    ...
    <div className="relative flex h-screen overflow-hidden bg-white">
      ...
      {view === "list" ? (
        <GradingListView
          ...
          onNext={handleNext}
          isNextDisabled={
            isAdvancing || totalCount === 0 || confirmedCount < totalCount
          }
        />
            ...
            <div>
              ...
              {gradingJobProgress && gradingJobProgress.total > 0 ? (
                <>
                  ...
                  <p className="text-[13.5px] text-[#71757e]">
                    {gradingJobProgress.current} / {gradingJobProgress.total}{" "}
                    채점 완료
                  </p>
                </>
              ) : (
