...
import {
  problemsApi,
  type ModelAnswerOcrRequest,
  type ModelAnswerUpdateRequest,
} from "@/api/problems";

export function useModelAnswerOcr(
  examId: number,
  ...
  const [modelAnswerBlobUrl, setModelAnswerBlobUrl] = useState<string | null>(
    null,
  );
  ...
  const uploadFile = async (file: File) => {
    const res = await problemsApi.getModelAnswerUploadUrl(
      examId,
      file.name,
      file.type,
    );
