...
export function useStep6(examId: number) {
  ...
  const progress: GradingProgressResponse | null | undefined =
    progressQuery.data;
  ...
  const modelAnswersQuery = useQuery({
    ...
    enabled:
      !!selectedProblem &&
      view === "detail" &&
      AUTO_TYPES.includes(selectedProblem.type),
  });
  const allModelAnswers: ModelAnswerResponse[] = modelAnswersQuery.data ?? [];
  const selectedModelAnswer =
    allModelAnswers.find(
      (ma) => ma.problem_id === selectedProblem?.problem_id,
    ) ?? null;
  ...
  const gradesQuery = useQuery({
    ...
    queryFn: () =>
      gradingApi
        .getProblemGrades(selectedProblem!.problem_id)
        .then((r) => r.data ?? []),
  ...
  const runGrading = useCallback(async () => {
    ...
    try {
      const res = await gradingApi.runForProblem(
        examId,
  ...
  const confirmAllMutation = useMutation({
    ...
    onSuccess: (res) => {
      toast.success(
        `${res.data?.confirmed_count ?? 0}명 채점이 확정되었습니다`,
      );
      qc.invalidateQueries({
        queryKey: ["grades", selectedProblem?.problem_id],
      });
  ...
  const navGrade = useCallback(
    (delta: number) =>
      setSelectedGradeIdx((i) =>
        Math.max(0, Math.min(i + delta, grades.length - 1)),
      ),
    [grades.length],
  );
  ...
  const navSheet = useCallback(
    (delta: number) =>
      setSelectedSheetIdx((i) =>
        Math.max(0, Math.min(i + delta, sheetsData.length - 1)),
      ),
    [sheetsData.length],
  );
  ...
  const selectedSheet: AnswerSheetResponse | null =
    sheetsData[selectedSheetIdx] ?? null;
  ...
  const selectedProblemLanguage: ProgrammingLanguage | null =
    problemsList.find((p) => p.problem_id === selectedProblem?.problem_id)
      ?.language ?? null;
  ...
  const { data: sheetDownloadRes } = useQuery({
    ...
    queryFn: () =>
      sheetsApi.getDownloadUrl(selectedAnswerSheetId!).then((r) => r.data),
  ...
  return {
    ...
    deleteGrades: () =>
      selectedProblem &&
      deleteGradesMutation.mutate(selectedProblem.problem_id),
