...
export function useStep5(examId: number) {
  ...
  const [studentOcrStudentId, setStudentOcrStudentId] = useState<number | null>(
    null,
  );
    ...
    onComplete: () => {
      ...
      if (rerunStudentIdRef.current !== null) {
        qc.invalidateQueries({
          queryKey: ["ocr-results", rerunStudentIdRef.current],
        });
      ...
      qc.setQueryData(
        ...
        (old: ApiResponse<OcrResultResponse[]> | undefined) => {
          ...
          return {
            ...old,
            data: old.data.map((r) =>
              r.ocr_result_id === resultId ? { ...r, ...patch } : r,
            ),
          };
        },
      );
  ...
  const saveText = useCallback(() => {
    ...
    updateMutation.mutate({
      resultId: selectedResult.ocr_result_id,
      body: { text: localText },
    });
