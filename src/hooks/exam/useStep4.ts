...
export function useStep4(examId: number) {
  ...
  const effectiveActiveProblemId =
    activeProblemId ?? problems[0]?.problem_id ?? null;

  const { data: downloadRes } = useQuery({
    queryKey: ["sheet-download", selectedSheet?.answer_sheet_id],
    queryFn: () =>
      sheetsApi
        .getDownloadUrl(selectedSheet!.answer_sheet_id)
        .then((r) => r.data),
  ...
  const canApplyTemplate =
    isFixedMode && (isTemplateApplied || localRegions.length > 0);
  ...
  const deleteRegionMutation = useMutation({
    mutationFn: ({ regionId }: DeleteRegionVars) =>
      regionsApi.deleteRegion(regionId),
    onSuccess: (_, { sheetId }) =>
  ...
  const saveAndApplyMutation = useMutation({
    mutationFn: async () => {
      const templateSource = isTemplateApplied
        ? firstSheetRegions
            .filter(
              (r): r is AnswerRegionResponse & { bbox_region: Region } =>
                !!r.bbox_region,
            )
            .map((r) => ({
    ...
    onError: (err: Error) =>
      toast.error(err.message || "템플릿 적용에 실패했습니다."),
  });
  ...
  const handleDrawComplete = useCallback(
    (selection: DrawSelection) => {
      ...
      const polygon_points =
        selection.shape === "LASSO" ? selection.polygon_points : undefined;

      // 영역 지정 후 다음 문제로 자동 전진
      const currentIdx = problems.findIndex(
        (p) => p.problem_id === effectiveActiveProblemId,
      );
      ...
      setLocalRegions((prev) => [
        ...
        {
          tempId: nextTempId(),
          problem_id: effectiveActiveProblemId,
          bbox_region,
          shape,
          polygon_points,
        },
  ...
  const deleteLocalRegion = useCallback(
    (tempId: string) =>
      setLocalRegions((prev) => prev.filter((r) => r.tempId !== tempId)),
    [],
  );
  ...
  const mappingList = useMemo(() => {
    ...
    if (isFineTuneMode) {
      ...
      const pendingItems = pendingRegions.map((r, i) =>
        toItem(r, serverItems.length + i),
      );
