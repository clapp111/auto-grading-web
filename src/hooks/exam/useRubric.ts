...
import {
  problemsApi,
  type RubricCreateRequest,
  type RubricUpdateRequest,
} from "@/api/problems";
...
export function useRubric(problemId: number) {
  ...
  const invalidate = () =>
    qc.invalidateQueries({ queryKey: ["rubric", problemId] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (rubricId: number) =>
      problemsApi.deleteRubricCriteria(rubricId),
    onSuccess: invalidate,
