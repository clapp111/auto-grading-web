...
export const examsApi = {
  ...
  get: (examId: number) =>
    apiClient
      .get<ApiResponse<ExamResponse>>(`/exams/${examId}`)
      .then((r) => r.data),

  create: (body: ExamCreateRequest) =>
    apiClient
      .post<ApiResponse<ExamResponse>>("/exams", body)
      .then((r) => r.data),

  update: (examId: number, body: ExamUpdateRequest) =>
    apiClient
      .patch<ApiResponse<ExamResponse>>(`/exams/${examId}`, body)
      .then((r) => r.data),
  ...
  advance: (examId: number, fromStep: number) =>
    apiClient
      .post<ApiResponse<ExamResponse>>(`/exams/${examId}/advance`, {
        from_step: fromStep,
      })
      .then((r) => r.data),
};
